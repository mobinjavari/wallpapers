...
function parseBlob(filename: string, byteSize: number, parentTags: string[]): WallpaperMeta | null {
...
async function collectItems(
  ...
  for (const entry of entries) {
    if (entry.type === 'blob') {
      const item = parseBlob(entry.name, entry.object?.byteSize ?? 0, parentTags)
...
export async function fetchWallpapers(): Promise<WallpaperMeta[]> {
  ...
  const items = await collectItems(token, rootEntries, [])
