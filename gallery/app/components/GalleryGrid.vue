...
<template>
  <!-- Skeleton loading -->
  <div v-if="gallery.isLoading.value"
    class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
    <div v-for="i in 20" :key="i" class="aspect-4/3 rounded-2xl bg-slate-300 dark:bg-slate-600 animate-skeleton" />
  </div>

  <!-- Error state -->
  <div v-if="gallery.hasError.value" class="flex flex-col items-center justify-center py-28 gap-3">
    ...
    <p class="text-sm font-semibold text-slate-500 dark:text-slate-500">Failed to load wallpapers</p>
    <button
  ...
  <!-- Gallery grid -->
  <div v-if="!gallery.isLoading.value && !gallery.hasError.value && gallery.visibleItems.value.length > 0"
    class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
    <WallpaperCard v-for="(item, idx) in gallery.visibleItems.value" :key="item.rawUrl" :item="item" :index="idx"
      @openlightbox="(i) => gallery.openLightbox(i)" @opendownload="handleOpenDownload"
      @copylink="(i) => gallery.copyShareLink(i)" @tagclick="(tag) => gallery.triggerTagSearch(tag)" />
  </div>

  <!-- Empty state -->
  <div v-if="!gallery.isLoading.value && !gallery.hasError.value && gallery.filteredItems.value.length === 0"
    ...
    <p class="text-sm font-semibold text-slate-500 dark:text-slate-500">No wallpapers found</p>
    <button
  ...
  <div v-if="gallery.hasMore.value && !gallery.isLoading.value && !gallery.hasError.value" ref="scrollTriggerEl"
