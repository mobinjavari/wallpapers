...
<template>
  <div v-if="downloadModal.isOpen.value"
    class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 dm-fade-in">
    <!-- Backdrop -->
    <div class="fixed inset-0 bg-backdrop backdrop-blur-sm" role="presentation" @click="downloadModal.close()" />
    ...
    <div
      ...
      <div class="relative h-40 sm:h-44 overflow-hidden bg-surface-muted">
        <div class="absolute inset-0 dm-shimmer transition-opacity duration-300" :class="{ 'opacity-0': imgLoaded }" />
        <img :src="weservUrl(downloadModal.activeUrl.value, { w: 800, q: 82, output: 'webp' })"
          class="cinema-pan w-full h-full object-cover transition-opacity duration-300"
          :class="{ 'opacity-0': !imgLoaded, 'opacity-100': imgLoaded }" alt="Preview" @load="imgLoaded = true" />
      ...
      <div class="dm-body px-4 m-2">
        <div class="flex items-center gap-2 mb-4">
          <p class="text-sm font-bold text-fg truncate">{{ downloadModal.activeName.value }}</p>
          <span
