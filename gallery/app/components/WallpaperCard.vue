...
const thumbUrl = computed(() => weservUrl(props.item.rawUrl, { w: 500, q: 82, output: 'webp' }))
</script>
...
<template>
  <div
    ...
    <img :src="thumbUrl" :alt="item.name" loading="lazy" decoding="async" class="card-img w-full h-full object-cover" />
    ...
    <div class="card-center absolute inset-0 z-20 flex items-center justify-center">
      <a :href="`/wallpaper/${encodeURIComponent(item.name)}`"
        ...
        title="Open wallpaper" @click.prevent="emit('openlightbox', index)">
    ...
    <div class="card-top absolute top-2.5 right-2.5 z-20 flex flex-wrap gap-1 justify-end max-w-[80%]">
      <a v-for="tag in item.hashtags.slice(-4)" :key="tag" :href="`/tag/${encodeURIComponent(tag)}`"
    ...
    <div class="card-bottom absolute bottom-0 inset-x-0 p-2.5 z-20 flex items-end justify-between gap-2">
      <div class="min-w-0">
        <p class="text-[11px] font-semibold text-white leading-tight truncate">{{ item.name }}</p>
        <p class="text-[10px] font-bold text-white/40 uppercase mt-0.5">{{ item.ext }}</p>
      </div>
      <div class="flex items-center gap-1 shrink-0">
        <button
          ...
          title="Copy link" @click.stop="emit('copylink', item)">
        ...
        <a :href="`/download/${encodeURIComponent(item.name)}`"
          ...
          title="Download" @click.prevent="emit('opendownload', item)">
