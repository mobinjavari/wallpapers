...
const displayName = computed(() =>
  lightbox.currentItem.value ? humanizeWallpaperName(lightbox.currentItem.value.name) : '',
)
...
<template>
  ...
  >
    ...
    <div
      v-if="lightbox.currentItem.value && !isBrowserFullscreen"
      class="lb-top-left absolute z-50"
    >
    ...
    <!-- Counter -->
    <div
      v-if="!lightbox.imgLoading.value"
    ...
    <!-- Image -->
    <img
      :src="lightbox.imgSrc.value"
      :alt="lightbox.imgAlt.value"
      :class="[
    ...
    >
      ...
      <div class="flex items-center gap-2 shrink-0">
        <button
          class="btn-press w-9 h-9 rounded-xl bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white border border-white/10 flex items-center justify-center transition-all"
          title="Copy link"
          @click="gallery.copyShareLink(lightbox.currentItem.value!)"
        >
